...
test('it renders correctly in basic usage', function(assert) {
  ...
  assert.equal(banner.textContent.trim(), 'template block text', 'block-mode - banner has correct content');
});
...
test('it renders banner heading correctly', function(assert) {
  ...
  assert.notOk(banner.getAttribute('aria-labelledby'), 'banner without title - no aria-labelledby for banner');
  ...
  assert.ok(banner.hasAttribute('aria-labelledby'), 'banner with title - has aria-labelledby attribute');
  assert.equal(
...
test('it handles banner content correctly', function(assert) {
  ...
  assert.notOk(banner.getAttribute('aria-describedby'), 'banner without content - no aria-describedby for banner');
  ...
  assert.equal(content.textContent.trim(), contentText, 'banner with content - has correct content');
  assert.ok(banner.hasAttribute('aria-describedby'), 'banner with content - has aria-describedby attribute');
  assert.equal(
  ...
  assert.equal(content.textContent.trim(), contentText, 'banner with content (block) - has correct content');
  assert.ok(banner.hasAttribute('aria-describedby'), 'banner with content (block) - has aria-describedby attribute');
  assert.equal(
    banner.getAttribute('aria-describedby'),
    content.getAttribute('id'),
...
test('it handles banner status correctly', function(assert) {
  ...
  let banner = find(bannerSelector);
  banner.classList.forEach((bannerClass) => assert.notOk(/Polaris-Banner--status/.test(bannerClass), 'banner without status - has no status class'));
...
test('it supports `action` and `secondaryAction`', function(assert) {
  ...
  assert.notOk(secActionFired, 'banner with actions - clicking `action` button - does not trigger `secondaryAction`\'s action');

  mainActionFired = false;
  click(secondaryActionBtn);

  assert.ok(secActionFired, 'banner with actions - clicking `secondaryAction` button - trigger `secondaryAction`\'s action');
  assert.notOk(mainActionFired, 'banner with actions - clicking `secondaryAction` button - does not trigger `action`\'s action');
});
