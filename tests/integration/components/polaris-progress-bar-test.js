...
module('Integration | Component | polaris progress bar', function(hooks) {
  ...
  test('it renders the correct HTML when progress and size are set', async function(assert) {
    await render(hbs`{{polaris-progress-bar progress=23 size='small'}}`);

    let barNode = find(BAR_SELECTOR);
    let progressNode = find(
      buildNestedSelector(BAR_SELECTOR, PROGRESS_SELECTOR)
    );
    let indicatorNode = find(
      buildNestedSelector(BAR_SELECTOR, INDICATOR_SELECTOR)
    );
    let labelNode = find(
      buildNestedSelector(INDICATOR_SELECTOR, LABEL_SELECTOR)
    );
    ...
    assert.ok(
      progressNode,
      'the progress node is rendered inside the container'
    );
    assert.ok(
      indicatorNode,
    ...
    // Bar attributes
    assert.ok(
      barNode.classList.contains('Polaris-ProgressBar--sizeSmall'),
      'size class is added to the container'
    );
    ...
    assert.equal(
      indicatorNode.getAttribute('style'),
      percentStyle,
      'indicator width style is correct'
    );
  ...
  test('it renders a correctly-sized progress bar', async function(assert) {
    ...
    assert.ok(
      barNode.classList.contains('Polaris-ProgressBar--sizeMedium'),
      'no size - applies medium size class'
    );
    assert.notOk(
      barNode.classList.contains('Polaris-ProgressBar--sizeLarge'),
  ...
  test('it correctly handles out-of-bounds progress numbers', async function(assert) {
    ...
    assert.equal(
      progressNode.getAttribute('value'),
      '0',
      'negative progress value - value attribute limited to a min of 0'
    );
    assert.equal(
      indicatorNode.style.width,
      '0%',
      'negative progress value - indicator width limited to a min of 0%'
    );
    assert.equal(
      labelNode.textContent.trim(),
      '0%',
      'negative progress value - label limited to a min of 0% in progress label'
    );
    ...
    assert.equal(
      progressNode.getAttribute('value'),
      '100',
      'progress value over 100 - value attribute limited to a max of 100'
    );
    assert.equal(
      indicatorNode.style.width,
      '100%',
      'progress value over 100 - indicator width limited to a max of 100%'
    );
    assert.equal(
      labelNode.textContent.trim(),
      '100%',
      'progress value over 100 - label limited to a max of 100% in progress label'
    );
  });
});
