...
module('Integration | Component | polaris button', function(hooks) {
  ...
  test('renders the correct HTML', async function(assert) {
    ...
    await render(hbs`{{polaris-button text="Look at my text"}}`);

    buttons = findAll('button[type="button"].Polaris-Button');
    assert.equal(
      buttons.length,
      1,
      'basic button with text attribute - renders'
    );
    ...
    await render(hbs`{{#polaris-button}}{{/polaris-button}}`);
    ...
    await render(hbs`{{polaris-button primary=true}}`);
    ...
    await render(hbs`{{polaris-button destructive=true}}`);
    ...
    // Slim button.
    await render(hbs`{{polaris-button size="slim"}}`);
    ...
    // Large button.
    await render(hbs`{{polaris-button size="large"}}`);
    ...
    await render(hbs`{{polaris-button outline=true}}`);
    ...
    await render(hbs`{{polaris-button fullWidth=true}}`);
    ...
    await render(hbs`{{polaris-button plain=true}}`);
    ...
    await render(
      hbs`{{polaris-button accessibilityLabel="You can't see me!"}}`
    );
    ...
    await render(hbs`{{polaris-button ariaControls=ariaControls}}`);
    ...
    await render(hbs`{{polaris-button ariaExpanded=ariaExpanded}}`);
  ...
  test('it supports `icon`', async function(assert) {
    await render(hbs`{{polaris-button icon=icon text=text}}`);
  ...
  test('it supports `disclosure`', async function(assert) {
    await render(hbs`{{polaris-button text="Click me" disclosure=disclosure}}`);
  ...
  test('handles events correctly', async function(assert) {
    ...
    return focus(buttonSelector)
      .then(() => {
        assert.notOk(
          clickHandlerCalled,
          'after focus, click handler not fired'
        );
  ...
  test('it applies passed-in classes to the rendered element when rendering a button', async function(assert) {
    ...
    await render(hbs`{{polaris-button class=class}}`);
  ...
  test('handles the loading flag correctly', async function(assert) {
    await render(hbs`{{polaris-button
    ...
    assert.equal(
      spinners.length,
      0,
      'loading unset - does not render a spinner'
    );
