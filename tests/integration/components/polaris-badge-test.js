...
test('it renders the correct HTML in basic inline usage', function(assert) {
  ...
  assert.equal(
    ...
    'Inline badge',
    'renders the correct badge content'
  );
});
...
test('it renders the correct HTML when status is set', function(assert) {
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--statusSuccess'),
  ...
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'status set to success - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Success',
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--statusInfo'),
  ...
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'status set to info - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Info',
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--statusAttention'),
  ...
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'status set to attention - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Attention',
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--statusWarning'),
  ...
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'status set to warning - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Warning',
...
test('it renders the correct HTML when progress is set', function(assert) {
  ...
  assert.equal(
    badgePipComponent.length,
    0,
    'progress not set - does not render a pip component'
  );
  assert.equal(
    visuallyHiddenComponents.length,
    0,
    'progress not set - does not render a visually hidden component'
  );
  ...
  assert.equal(
    badgePipComponent.length,
    0,
    'progress set to default - does not render a pip component'
  );
  assert.equal(
    visuallyHiddenComponents.length,
    0,
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--progressIncomplete'),
  ...
  assert.equal(
    badgePipComponent.length,
    1,
    'progress set to incomplete - renders one pip component'
  );
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'progress set to incomplete - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Incomplete',
    'progress set to incomplete - renders correct visually hidden content'
  );
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--progressPartiallyComplete'),
  ...
  assert.equal(
    badgePipComponent.length,
    1,
    'progress set to partiallyComplete - renders one pip component'
  );
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'progress set to partiallyComplete - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Partially complete',
    'progress set to partiallyComplete - renders correct visually hidden content'
  );
  ...
  assert.ok(
    badge.classList.contains('Polaris-Badge--progressComplete'),
  ...
  assert.equal(
    badgePipComponent.length,
    1,
    'progress set to complete - renders one pip component'
  );
  assert.equal(
    visuallyHiddenComponents.length,
    1,
    'progress set to complete - renders one visually hidden component'
  );
  assert.equal(
    visuallyHiddenComponents[0].textContent.trim(),
    'Complete',
    'progress set to complete - renders correct visually hidden content'
  );
});
