...
const headerSelector = buildNestedSelector(pageSelector, 'div.Polaris-SkeletonPage__Header');
const contentSelector = buildNestedSelector(pageSelector, 'div.Polaris-SkeletonPage__Content');
...
test('it renders the correct page content', function(assert) {
  ...
  const blockContentSelector = buildNestedSelector(contentSelector, 'p#block-content');
...
test('it renders the page title correctly', function(assert) {
  ...
  const titleSelector = buildNestedSelector(headerSelector, 'div.Polaris-SkeletonPage__Title');
...
test('it renders secondary actions correctly', function(assert) {
  ...
  let header = find(headerSelector);
  assert.notOk(
    header.classList.contains('Polaris-SkeletonPage__Header--hasSecondaryActions'),
  ...
  secondaryActions.forEach((secondaryAction, index) => {
    assert.equal(
      ...
      `secondary action ${index + 1} - renders the action with the correct width`,
    );
